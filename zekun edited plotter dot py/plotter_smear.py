...
class BSDOSPlotter:
    ...
    def get_plot(self, bs: BandStructureSymmLine, dos: Dos | CompleteDos | None = None):
        ...
        bs_ax.set_ylabel("$E-E_F$(HES) / eV", fontsize=self.axis_fontsize, family=self.font)
        ...
        if dos:
            ...
            dos_xmax = max([max(el_dos[Element(el)].get_smeared_densities(self.sigma)[Spin.up][emin_idx:emax_idx]) * 1.05, abs(dos_xmin)])
