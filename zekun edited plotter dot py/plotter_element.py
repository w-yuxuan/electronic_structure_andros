...
class DosPlotter:
    ...
    def get_plot(
        ...
        colors = ['r',[0.5,0,0.5], "g","b","m","c","y"]

        ys = None
        all_densities = []
        all_energies = []
        plt = pretty_plot(12, 3)
        ...
        if invert_axes:
            ...
            plt.xlabel("Energies (eV)", fontsize = 24)
            if self._norm_val:
                plt.ylabel("Density of states (states/eV/Å³)")
            else:
                plt.ylabel("DOS (#/eV)",fontsize = 24)
    ...
    def get_plot(
        ...
        for ibs, bs in enumerate(self._bs):
            ...
            for sp in bs.bands:
                ...
                bs_label = "" if bs_labels is None else f"{bs_labels[ibs]} {sp.name}"
        ...
        plt.axhline(y=5.0764, c='b', ls='--', lw=3)
        if one_is_metal:
            e_min = -10
            e_max = 10
        ...
        # Main X and Y Labels
        plt.xlabel(r"$\mathrm{Wave\ Vector}$", fontsize=20)
...
class BSDOSPlotter:
    ...
    def get_plot(self, bs: BandStructureSymmLine, dos: Dos | CompleteDos | None = None):
        ...
        fig = plt.figure(figsize=self.fig_size,dpi=1200)
        ...
        if dos:
            ...
            for spin in (Spin.up, Spin.down):
                if spin in dos.densities:
                    ...
                    label = "sum" if spin == Spin.up else None
                    ...
                    if self.dos_projection is None:
                        ...
                        colors = self.customize_element
                        el_dos = dos.get_element_dos()
                        #elements_customize = elements[0]
                        #for idx, el in enumerate([elements[0],elements[1],elements[2],elements[3],elements[4],elements[5]]):
                        for idx, el in enumerate([elements[0]]):
