...
#%%
                    if self.dos_projection is None:
                        ...
                        #%% og: plot the atom-projected DOS
                        ...
                        colors = self.customize_color_order
                        el_dos = dos.get_element_dos()
                        ...
                        #for idx, el in enumerate([elements[1],elements[0],elements[2]]): # for 3 element czs. change to next line to plot only one element
                        #put the element with largest dos at the end, to normalize acc to that.
            ...
            dos_xmin = (
                0 if Spin.down not in dos.densities else -max(el_dos[Element(el)].densities[Spin.down][emin_idx : emax_idx + 1] * 1.05)
            )
            dos_xmax = max([max(el_dos[Element(el)].densities[Spin.up][emin_idx:emax_idx]) * 1.05, abs(dos_xmin)])

            # set up the DOS x-axis and add Fermi level line
            dos_ax.set_xlim(dos_xmin, dos_xmax)
            dos_ax.set_xticks(np.arange(int(dos_xmin), int(dos_xmax) , int((dos_xmax-dos_xmin)/4)))
            # Set tick font size
            dos_ax.tick_params(axis='x', labelsize=self.tick_fontsize)
            #dos_ax.set_xticks(range(0,20,10))
